...
The result object can handle multiple result sets form batches SQL or stored procedures. It is critical to remember that when calling each for the first time with a block while expecting multiple result sets will return each "row" of each result set. Calling each a second time with a block will yield each "set".
