...
The result object can handle multiple result sets form batched SQL or stored procedures. It is critical to remember that when calling each for the first time with a block while expecting multiple result sets will return each "row" of each result set. Calling each a second time with a block will yield each "set".

  set1, set2 = client.execute("EXEC sp_helpconstraint '[datatypes]'").each
  
  set1 # => [{"Object Name"=>"[datatypes]"}]
  set2 # => [{"status_for_replication"=>"(n/a)", "status_enabled"=>"(n/a)", 
              "delete_action"=>"(n/a)", "constraint_keys"=>"id", "update_action"=>"(n/a)", 
              "constraint_name"=>"PK__datatype__3213E83F68B35D82", 
